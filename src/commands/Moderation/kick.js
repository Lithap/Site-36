...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('kick')
        .setDescription('Kick a user from the server with a reason.')
    ...
    async execute(interaction, client) {
        ...
        try {
            await targetMember.kick(`Kicked by ${member.user.tag}: ${reason}`);
        } catch (error) {
        ...
        await interaction.reply({ content: `${targetUser} has been kicked from the server. Reason: ${reason}`, ephemeral: true });
        ...
        if (logChannel) {
            const embed = new EmbedBuilder()
                ...
                .setDescription(`**Action Taken:** Kick\n` +
                                `**Executed By:** ${member} (${member.user.tag})\n` +
                                `**Target User:** ${targetUser} (${targetUser.tag})\n` +
                                `**Reason:** ${reason}\n` +
                                `**Server:** ${interaction.guild.name}\n` +
                                `**Server ID:** ${interaction.guild.id}`)
                .setThumbnail(interaction.guild.iconURL({ dynamic: true })) // Server icon
                .setAuthor({
